...
def recoger_datos(parque, evento_activo, ahora):
    ...
    continente = parque.get("continent", "Unknown")

    live_url = f"{BASE_URL}/{entity_id}/live"

    # Normalizar nombres de carpetas
    nombre_parque = nombre.replace(" ", "")
    nombre_archivo = f"{nombre_parque}_{ahora.date().isoformat()}.csv"

    # data/Europe/DisneyAdventureWorld/
        ...
        with open(ruta_archivo, mode="a", newline="", encoding="utf-8") as csvfile:
            ...
            for ride in atracciones:
                ...
                writer.writerow([
                    ...
                    ride.get("name", ""),
                    ride.get("status", ""),
                    ride.get("queue", {}).get("STANDBY", {}).get("waitTime", ""),
                    evento_activo
                ])
